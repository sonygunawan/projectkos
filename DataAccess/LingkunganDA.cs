...
namespace LihatKos.DataAccess
{
    public class LingkunganDA
    {
        ...
        public List<LingkunganData> GetAllLingkungan()
        {
            try
            {
                List<LingkunganData> retVal = new List<LingkunganData>();

                DbCommand dbCommand = dbConnection.GetStoredProcCommand(db, "dbo.LIK_GetAllLingkungan");
                using (IDataReader dataReader = db.ExecuteReader(dbCommand))
                {
                    while (dataReader.Read())
                    {
                        LingkunganData data = new LingkunganData();
